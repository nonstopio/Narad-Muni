...
- Add missing `mcp:compile` step to the release workflow so the MCP server is built before packaging

## [1.7.1] - 2026-03-01

### ✨ Features

- Add MCP server with stdio transport, auto-registration with Claude Code, and status API endpoint
- Update MCP server configuration and upgrade SDK version

### 🐛 Bug Fixes

- Add missing sign-native-modules script required by the CI postinstall step
...
- Fix SQLite `IOERR_SHORT_READ` (error 522) crash in Electron app caused by incompatible WAL/SHM file state between better-sqlite3 (migrations) and Prisma's native Rust engine (queries). The fix checkpoints and removes WAL artifacts before Prisma connects, eliminates the scheduler's separate SQLite connection, and ensures PrismaClient is always cached as a singleton
